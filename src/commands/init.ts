...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  try {
    ...
    spinner.succeed(chalk.green(`项目检测完成: ${projectInfo.type} (${projectInfo.language})`));
    ...
    if (options.template) {
      ...
      console.log(chalk.blue(`使用推荐模板: ${recommendedTemplates.join(', ')}`));
    } else {
      // 交互式选择模板
 ...
 * 交互式选择模板
 */
async function selectTemplates(recommendedTemplates: string[]): Promise<string[]> {
...
function displayCompletionInfo(
  ...
  console.log(`  - 运行 ${chalk.green('cursor-rules add <rule-name>')} 添加更多规则`);
