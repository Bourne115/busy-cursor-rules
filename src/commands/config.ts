 ...
 * 配置管理命令
 */
export async function configCommand(options: ConfigOptions & { set?: string; get?: string }): Promise<void> {
  try {
    if (options.reset) {
      await resetConfig();
    ...
    console.log(`  ${chalk.green('cursor-rules config --set key=value')} 设置配置`);
